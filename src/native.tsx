...
import { GestureState, Props, SetSpringTarget, Vector } from 'types';
import { normalize, pythagoras } from 'utils';
import { reactNativeSettings as settings } from 'utils/constants';
...
const animateOut = async ( gesture: Vector, setSpringTarget: SetSpringTarget ) => {
...
const animateBack = ( setSpringTarget: SetSpringTarget ) => {
    // translate back to the initial position
    return new Promise( ( resolve ) => {
        setSpringTarget.current[ 0 ].start( { x: 0, y: 0, rot: 0, config: physics.animateBack, onRest: resolve } as any )
    } )
}

const getSwipeDirection = ( property: Vector ) => {
...
const TinderCard = forwardRef(
    (
        { flickOnSwipe = true, children, onSwipe, onCardLeftScreen, className, preventSwipe = [], swipeRequirementType = 'velocity', swipeThreshold = settings.swipeThreshold, onSwipeRequirementFulfilled, onSwipeRequirementUnfulfilled }: Props,
        ref: any
    ) => {
        ...
        const handleSwipeReleased = React.useCallback(
            async ( setSpringTarget: any, gesture:any ) => {
        ...
        const panResponder = React.useMemo(
            () =>
                PanResponder.create( {
                    ...
                    onStartShouldSetPanResponder: ( evt: any, gestureState: GestureState ) => true,
                    onStartShouldSetPanResponderCapture: ( evt: any, gestureState: GestureState ) => true,
                    onMoveShouldSetPanResponder: ( evt: any, gestureState: GestureState ) => true,
                    onMoveShouldSetPanResponderCapture: ( evt: any, gestureState: GestureState ) => true,

                    onPanResponderGrant: ( evt: any, gestureState: GestureState ) => {
                    ...
                    onPanResponderMove: ( evt: any, gestureState:GestureState ) => {
                        // Check fulfillment
                    ...
                    onPanResponderTerminationRequest: ( evt: any, gestureState: GestureState ) => {
                        return true
                    },
                    onPanResponderRelease: ( evt:any, gestureState: GestureState ) => {
            ...
            <AnimatedView
                ...
                style={{
                    transform: [
                        { translateX: x },
                        { translateY: y },
                        { rotate: rot.to( ( rot:number ) => `${ rot }deg` ) }
                    ]
                }}
                className={className}
            >
